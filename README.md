...
## Group text-form escrow flow (new)

A second, simpler deal-creation flow lives alongside the wizard above, driven entirely
by group chat commands/buttons and a copy-paste form message:

- **Group-only.** `middleware/groupOnly.js` refuses any DM update with a notice —
  the bot only functions inside group chats.
- **`/help` mirrors `/start`** — both send the same welcome message + main menu.
- **`/createdeal`** (and the `📝 Create Deal` button) send the blank
  `NFT MRTK ESCROW FORM` template. The user copies it, fills in every `➤` field, and
  sends it back as one message.
- **Anti-impersonation check.** A filled form is only processed if it contains the
  bot's own `@username` tag (fetched live via `getMe()`). Messages that look like a
  form attempt but are missing that tag, or a required field, get a specific
  rejection reason instead of being silently accepted — see `handlers/groupForm.js`.
- **Agree buttons.** The bot reposts the parsed form with `✅ Seller Agree` /
  `✅ Buyer Agree` buttons. A tap only counts if the tapper's Telegram `@username`
  matches the one typed into that field — anyone else gets an alert, not silent
  failure.
- **Escrow ID.** Issued the moment both sides have agreed (`group_deals.escrow_id`,
  format `ESC-XXXXXX`) — before any admin has acted — and reported to both parties.
  Anyone can check it with `/dealstatus <escrow id>`.
- **Admin/owner acceptance.** Once both agree, a `🛡️ Accept Deal` button appears.
  Only a configured `ADMIN_IDS` member or the group's creator can tap it. Whoever
  claims it first is the only one who can subsequently tap `➡️ Forward Deal` or
  `🗑️ Discard Deal` — enforced both by hiding/showing buttons and by a server-side
  check in `services/groupDealService.js` (`claimed_by`).
- Storage is a dedicated `group_deals` / `group_deal_events` table
  (`database/migrations/002_group_form_deals.sql`), kept separate from the older
  `deals` table so the wizard-based flow above is untouched. Run `npm run migrate`
  to apply it.
