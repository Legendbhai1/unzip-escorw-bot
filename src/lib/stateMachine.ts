...
const transitions: StateTransition[] = [
  ...
  { from: "JOINED",            to: "AWAITING_FUNDING",  triggeredBy: "SYSTEM",  condition: "Both parties accepted" },
  { from: "AWAITING_FUNDING",  to: "FUNDED",            triggeredBy: "SYSTEM",  condition: "Buyer balance locked + buyer fee collected" },
  ...
  { from: "RELEASE_PENDING",   to: "COMPLETED",         triggeredBy: "SYSTEM",  condition: "Ledger transfer executed" },

  // ── Dispute path ──
  // Allowed from FUNDED, IN_PROGRESS, DELIVERED, RELEASE_PENDING
  ...
  { from: "UNDER_REVIEW",      to: "RELEASED",          triggeredBy: "ADMIN",   condition: "Release to seller (admin)" },
  { from: "UNDER_REVIEW",      to: "REFUNDED",          triggeredBy: "ADMIN",   condition: "Refund buyer (admin)" },
  // RELEASED is a terminal state used for admin dispute resolution.
  // Normal flow goes RELEASE_PENDING -> COMPLETED directly.

  // ── Expiration ──
  { from: "AWAITING_FUNDING",  to: "EXPIRED",           triggeredBy: "SYSTEM",  condition: "Funding timeout" },
  ...
  { from: "AWAITING_FUNDING",  to: "CANCELLED",         triggeredBy: "BUYER",   condition: "Buyer cancels before funding" },
  { from: "AWAITING_FUNDING",  to: "CANCELLED",         triggeredBy: "SELLER",  condition: "Seller cancels before funding" },
];
