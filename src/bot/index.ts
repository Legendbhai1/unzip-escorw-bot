...
bot.on("callback_query:data", async (ctx) => {
  ...
  if (data === "menu:how_it_works") {
    await ctx.editMessageText(
      "<b>HOW IT WORKS</b>\n\n1. Create a deal and invite the other party\n2. Both parties agree to the terms\n3. Buyer deposits funds into escrow\n4. Seller delivers the item/service\n5. Buyer confirms and releases the funds\n6. Seller receives the payment\n\nIf anything goes wrong, either party can open a dispute.",
      { reply_markup: backToMain }
    );
    return;
  }
  ...
  // ── Deal: Check payment ──
  if (data.startsWith("deal:check_payment:")) {
    const dealId = data.split(":")[2];
    await showDealStatus(ctx, dealId);
    return;
  }
  ...
  if (data?.startsWith("cat:")) {
    ...
    await ctx.reply(
      ...
      `Escrow Fee: 1% (${(parseFloat(s.createDealAmount ?? "0") * 0.01).toFixed(2)} ${s.createDealAsset})\n` +
