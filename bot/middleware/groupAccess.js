...
async function groupAccess(ctx, next) {
  ...
  let allowed;
  try {
    allowed = await groupAccessService.isAllowed(chat.id);
  } catch (err) {
    // Fail safe: if the allowlist check itself breaks (e.g. missing migration),
    // don't brick the entire group — log it and let admins through so they can fix it.
    console.error('groupAccess check failed:', err.message);
    return ctx.state.isAdmin ? next() : undefined;
  }
