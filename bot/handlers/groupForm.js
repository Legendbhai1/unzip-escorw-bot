...
const { groupFormAgreeKeyboard, groupFormAdminClaimKeyboard, groupFormAdminActionKeyboard } = require('../keyboards');
...
function looksLikeFormAttempt(text) {
  ...
  return bulletCount >= 3 || /escrow\s*form/i.test(text);
}

function hasOfficialTag(text) {
  const tag = officialTag().toLowerCase();
...
function filledFormText(deal) {
  ...
  if (deal.status === 'CLAIMED') {
    ...
    text += `\n\n✅ Deal accepted and moved forward by an admin.`;
...
function register(bot) {
  ...
  bot.on('text', async (ctx, next) => {
    ...
    if (!hasOfficialTag(text)) {
      await ctx.reply(
        `⚠️ This form is missing the official verification tag (@${officialTag()}) — I can't process forms ` +
          `from other bots or edited templates. Use /createdeal to get the correct one.`
      );
      return;
    }
    ...
    if (result.invalid) {
      await ctx.reply(`⚠️ ${result.invalid}. Use /createdeal to get a fresh form.`);
      return;
    }
    if (result.missing && result.missing.length) {
      await ctx.reply(
        `⚠️ Missing or empty fields: ${result.missing.join(', ')}.\nUse /createdeal to get a fresh form and fill in every ➤ field.`
      );
      return;
    }
    ...
    const sent = await ctx.reply(filledFormText(deal), {
      parse_mode: 'HTML',
  ...
  bot.action(/^gform:claim:(\d+)$/, async (ctx) => {
    ...
    try {
      ...
      console.error(err);
      return ctx.answerCbQuery(`⚠️ ${err.message === 'Deal is not awaiting admin acceptance.' ? err.message : 'Could not claim deal.'}`, {
        show_alert: true,
      });
    }
  });
...
module.exports = { register, sendCreateDealForm };
